...
export function KeywordDialog({
  ...
  useEffect(() => {
    ...
    if (initial) {
      const firstLink =
        Array.isArray(initial.links) && initial.links.length > 0
          ? (initial.links[0] as Record<string, unknown>)
          : null;
      setVals({
        ...initial,
        clientId: initial.clientId != null ? String(initial.clientId) : "",
        businessId:
          initial.businessId != null ? String(initial.businessId) : "",
        ...
        initialRankReportLink:
          firstLink?.initialRankReportLink ??
          initial.initialRankReportLink ??
          "",
        currentRankReportLink:
          firstLink?.currentRankReportLink ??
          initial.currentRankReportLink ??
          "",
      });
    } else {
      setVals({
  ...
  const businessOptions = (businesses ?? []).filter(
    (b) => !selectedClientId || String(b.clientId) === selectedClientId,
  );
  const campaigns = (plans ?? []).filter(
    (p) =>
  ...
  const selectedBusiness = businessOptions.find(
    (b) => String(b.id) === selectedBusinessId,
  );
  const selectedClient = (clients ?? []).find(
    (c) => String(c.id) === selectedClientId,
  );
  const selectedCampaign = campaigns.find(
    (c) => String(c.id) === (vals.aeoPlanId as string),
  );
      ...
      <DialogContent className="w-screen h-screen max-w-full max-h-screen rounded-none border-0 bg-white dark:bg-slate-900 flex flex-col p-0 gap-0">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 max-w-3xl w-full mx-auto space-y-5">
          {lockContext ? (
            <div className="rounded-lg border border-slate-200 bg-slate-50 dark:bg-slate-800/60 dark:border-slate-700 px-4 py-3 flex flex-wrap gap-x-8 gap-y-2 text-sm">
              ...
              <div>
                ...
                <span className="text-black dark:text-white font-semibold">
                  {selectedCampaign?.name ?? selectedCampaign?.planType ?? "—"}
                </span>
              </div>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <Select
                    value={(vals.aeoPlanId as string) || ""}
                    onValueChange={(v) => set("aeoPlanId", v)}
          ...
          <div className="space-y-1.5">
            ...
            <Input
              className="bg-slate-50 dark:bg-slate-900 border-slate-300 dark:border-slate-700 h-11 text-base text-black dark:text-white"
          ...
          <div className="space-y-1.5">
            <Label className="text-sm uppercase tracking-widest text-black dark:text-white font-bold">
              Keyword Types <span className="text-red-600">*</span>
            </Label>
            <Select
              value={String(vals.keywordType)}
              onValueChange={(v) => set("keywordType", v)}
            >
                ...
                <div className="col-span-2 space-y-1.5">
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="Client website blog post">
                        Client website blog post
                      </SelectItem>
                      <SelectItem value="External article">
                        External article
                      </SelectItem>
            ...
            {[
              {
                k: "isPrimary",
                label: "Primary (1st)",
                sub: "Mark as primary keyword",
                checked:
                  vals.isPrimary === "1" ||
                  vals.isPrimary === 1 ||
                  vals.isPrimary === true,
                onChange: (v: boolean) => set("isPrimary", v ? "1" : "0"),
                cls: "data-[state=checked]:bg-amber-500",
              },
              {
                k: "isActive",
                label: "Active",
                sub: "Include in campaigns",
                checked: vals.isActive !== false,
                onChange: (v: boolean) => set("isActive", v),
                cls: "data-[state=checked]:bg-emerald-500",
              },
            ].map((row) => (
              <div
                key={row.k}
                className="flex items-center gap-3 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg p-4"
              >
        ...
        <div className="shrink-0 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-6 py-5">
          <div className="flex gap-3 max-w-3xl mx-auto">
            <Button
              variant="outline"
              className="flex-1 border-slate-300 dark:border-slate-600 text-black dark:text-white hover:bg-slate-100 dark:hover:bg-slate-800 text-base font-bold h-12"
            ...
            <Button
              className="flex-1 gap-2 text-base font-bold h-12"
              disabled={
                saving ||
                !(vals.keywordText as string)?.trim() ||
                !vals.clientId ||
                !vals.businessId ||
                !vals.aeoPlanId
              }
