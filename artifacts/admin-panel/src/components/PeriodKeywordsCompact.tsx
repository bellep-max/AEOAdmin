...
export function PeriodKeywordsCompact({
  period,
  clientId,
  businessId,
  aeoPlanId,
  ...
  const grouped = useMemo(() => {
    const byKeyword = new Map<
      number,
      { keyword: PeriodRow; platforms: PeriodRow[] }
    >();
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        <Badge variant="outline">
          {grouped.length} keyword{grouped.length !== 1 ? "s" : ""}
        </Badge>
        {counts.improved > 0 && (
        ...
        {counts.steady > 0 && (
          <Badge variant="outline">= {counts.steady}</Badge>
        )}
        {counts.newCount > 0 && (
              ...
              return (
                <div key={keyword.keywordId} className="px-4 py-3">
                  ...
                  <div className="space-y-1">
                    {sorted.map((p) => (
                      <div
                        key={`${p.keywordId}-${p.platform}-row`}
                        className="grid grid-cols-12 gap-2 items-center text-xs pl-1 py-1"
                      >
