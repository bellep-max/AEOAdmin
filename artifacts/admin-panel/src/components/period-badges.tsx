...
export function StatusBadge({ status }: { status: Status }) {
  const map: Record<
    Status,
    { bg: string; text: string; label: string; icon: ReactElement }
  > = {
    improved: {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full border text-[10px] font-semibold ${p.bg} ${p.text}`}
    >
      {p.icon}
      {p.label}
    </span>
...
export function FreshnessBadge({ freshness }: { freshness: Freshness }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-full border text-[10px] font-semibold ${p.bg} ${p.text}`}
    >
      {p.label}
    </span>
  );
