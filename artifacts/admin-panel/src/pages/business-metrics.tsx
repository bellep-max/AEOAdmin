  ...
  return (
    <td className="px-3 py-0 border-l border-border/30 align-middle">
      <div className="flex flex-col items-center justify-center py-3 gap-1">
        <span className={`text-xl font-bold font-mono leading-none ${valCls}`}>{val}<span className="text-xs font-normal text-muted-foreground">%</span></span>
...
export default function BusinessMetrics() {
  ...
  function SortTh({ label, sortK, className }: { label: string; sortK: SortKey; className?: string }) {
    ...
    return (
      <th
        className={`px-3 py-3 text-center text-[10px] uppercase tracking-wider cursor-pointer select-none whitespace-nowrap border-l border-border/30 transition-colors hover:bg-muted/30 ${active ? "text-primary" : "text-muted-foreground/50"} ${className ?? ""}`}
                        ...
                        <div className="flex items-start gap-2.5">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                              <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${bm.client.status === "active" ? "bg-emerald-500" : "bg-muted-foreground/30"}`} />
                              <span className="text-[10px] text-muted-foreground capitalize">{bm.client.status}</span>
                              {bm.client.plan && (
                                <Badge variant="outline" className="text-[9px] h-4 px-1.5 text-muted-foreground/60 border-border/40">{bm.client.plan}</Badge>
                              )}
                            ...
                            <div>
                              ...
                              <div className="flex gap-4 text-xs">
                                ...
                                <div>
                                  ...
                                  <p className={`font-semibold ${bm.monthlyTarget - bm.sessionTotal > 0 ? "text-amber-400" : "text-emerald-400"}`}>
                            ...
                            <div>
                              ...
                              <div className="flex gap-4 flex-wrap text-xs">
                                {bm.deviceRotation.withDevice ? (
                                  <span className="text-muted-foreground/60">{bm.deviceRotation.uniqueDevices} unique devices / {bm.deviceRotation.withDevice} sessions</span>
                                ) : null}
                                {bm.ipRotation.withProxy ? (
                                  <span className="text-muted-foreground/60">{bm.ipRotation.uniqueProxies} unique proxies / {bm.ipRotation.withProxy} sessions</span>
                                ) : null}
                                {bm.promptAccuracy.total ? (
                                  <span className="text-muted-foreground/60">{bm.promptAccuracy.withPrompt} prompts / {bm.promptAccuracy.total} sessions</span>
                                ) : null}
                              </div>
                            </div>
                          </div>
              ...
              <tr>
                ...
                {[
                  ...
                  <td key={i} className="px-3 py-3 text-center border-l border-border/30">
                    {val !== null ? (
                      <span className={`text-sm font-bold font-mono ${val >= tgt ? "text-emerald-400" : val >= tgt * 0.8 ? "text-amber-400" : "text-destructive"}`}>
                        {val}%
                      </span>
                    ) : (
