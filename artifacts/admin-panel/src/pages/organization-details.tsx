...
function NotesCell({
  ...
  return (
    <>
      ...
      >
        <StickyNote
          className={`w-3.5 h-3.5 mt-0.5 flex-shrink-0 ${client.notes ? "text-amber-500" : "text-slate-300 group-hover:text-primary"}`}
        />
      ...
      >
        <DialogContent className="sm:max-w-[480px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Add any important notes about this organisation.
            </DialogDescription>
          </DialogHeader>
          <Textarea
            ref={textareaRef}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Overview of all business accounts — subscriptions, contacts,
              creation history &amp; notes
            </p>
          </div>
        </div>
        ...
        <div className="hidden md:flex gap-4">
          {[
            ...
            <div
              key={label}
              className="text-center px-4 py-2 rounded-xl bg-muted/50 border min-w-[72px]"
            >
          ...
          <TableHeader>
            <TableRow className="bg-muted/40">
              {(
                ...
                ] as { key: keyof OrgClient; label: string }[]
              ).map(({ key, label }) => (
                <TableHead
                  key={key}
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell className="align-top py-3 min-w-[160px]">
                      {(c.contactEmail ?? c.billingEmail) ? (
                        <div className="flex items-center gap-1.5 text-sm">
                          ...
                          <span
                            className="truncate max-w-[160px]"
                            title={c.contactEmail ?? c.billingEmail ?? ""}
                          >
                            {c.contactEmail ?? c.billingEmail}
                          </span>
                        </div>
                    ...
                    <TableCell className="align-top py-3">
                      {plan ? (
                        <span
                          className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold border ${plan.badgeClass} whitespace-nowrap`}
                        >
                          {c.planName}
                        </span>
                      ) : (
      ...
      <p className="text-xs text-muted-foreground px-1">
        {filtered.length} of {clients.length} organisation
        {clients.length !== 1 ? "s" : ""}
