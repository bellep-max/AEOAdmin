 ...
 * @page /scaling
 *
 * Scaling Plan page — displays the four-phase growth roadmap as an alternating
 * vertical timeline. Each milestone card shows target company count, devices
 * required vs available, a status badge, and operational notes.
 *
 * Data source: GET /api/scaling/plan
 * The `currentCompanies` field in each milestone is live from the DB;
 * everything else is static configuration defined in the route.
 *
 * Phases:
 *   1. Current Network Testing (20 companies, Apr 2026)
 *   2. Network Expansion      (50 companies, Apr 7 2026)
 *   3. Hardware Procurement   (80 companies, Apr 14 2026)
 *   4. May Scale              (150 companies, May 2026)
 */
...
export default function Scaling() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ── Timeline ────────────────────────────────────────────────────────
          Desktop: alternating left/right layout with a centre spine line.
          Mobile:  single-column with a left-edge spine line and offset dot.
      ─────────────────────────────────────────────────────────────────── */}
      <div className="relative pl-8 md:pl-0">
        {/* Vertical spine — desktop (centred) */}
        ...
        <div className="space-y-8 md:space-y-12 relative z-10">
          {isLoading ? (
            ...
            milestones?.map((milestone, index) => {
              // Even-indexed milestones go on the left (desktop); odd on the right
              ...
              return (
                <div
                  key={milestone.id}
                  className={`flex flex-col md:flex-row gap-4 md:gap-8 items-start md:items-center w-full ${isEven ? "" : "md:flex-row-reverse"}`}
                >
                  {/* ── Milestone card ── */}
                  <div className={`flex-1 w-full ${isEven ? "md:text-right" : "md:text-left"} pt-2 md:pt-0`}>
                    <Card className={`w-full md:max-w-md ${isEven ? "md:ml-auto" : "md:mr-auto"} ${
                      isActive
                        ? "border-primary/50 shadow-[0_0_15px_rgba(37,99,235,0.15)]"  // Glow on active phase
                        : "bg-card/50"
                    }`}>
                      <CardHeader className="pb-3">
                        ...
                        <div className={`flex items-center justify-between ${isEven ? "md:flex-row-reverse" : ""}`}>
                          ...
                          {/* Status badge — color-coded: green=done, blue=active, grey=upcoming */}
                      ...
                      <CardContent className="space-y-4">
                        {/* ── Stats grid: target clients + device procurement ── */}
                        <div className="grid grid-cols-2 gap-4 bg-muted/30 p-3 rounded-lg border">
                          ...
                          <div>
                            ...
                            {/* Amber = insufficient devices procured, Green = sufficient */}
                        ...
                        {/* Operational notes (hardware tasks, search volumes, etc.) */}
                        {milestone.notes && (
                  ...
                  {/* ── Timeline node icon ── */}
                  {/* Green circle=done, pulsing blue dot=active, grey clock=upcoming */}
                  ...
                  {/* Empty spacer to balance the two-column grid on desktop */}
