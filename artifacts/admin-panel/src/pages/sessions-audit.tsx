  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Audit-mode ranking checks from the executor — one row per keyword ×
            platform.
          </p>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
            ...
            <div className="space-y-1">
              ...
              >
                <SelectTrigger>
                  <SelectValue
                    placeholder={clientId == null ? "Pick client first" : "All"}
                  />
                </SelectTrigger>
                <SelectContent className={POPOVER_MAX_H}>
                    ...
                    (data?.logs ?? []).map((l) => (
                      ...
                      >
                        <TableCell className="whitespace-nowrap text-xs">
                          {fmtDateTime(l.timestamp ?? l.createdAt)}
                        </TableCell>
                        ...
                        <TableCell className="text-sm">
                          {l.mentioned === "yes" ? <Badge>Yes</Badge> : "—"}
                        </TableCell>
                        <TableCell className="text-sm">
          ...
          {open && (
            <>
              <SheetHeader>
                ...
                <SheetDescription>
                  {fmtDateTime(open.timestamp ?? open.createdAt)} ·{" "}
                  {open.platform ?? "—"}
                </SheetDescription>
              </SheetHeader>
              <div className="mt-6 space-y-5 text-sm">
                ...
                <Section title="Run">
                  ...
                  <Row
                    label="Device"
                    value={
                      open.device ?? (open.deviceId ? `#${open.deviceId}` : "—")
                    }
                  />
                  <Row
                    label="Duration"
                ...
                <Section title="Ranking">
                  <Row
                    label="Position"
                    value={
                      open.rankPosition != null ? `#${open.rankPosition}` : "—"
                    }
                  />
                ...
                <Section title="Proxy">
                  ...
                  <Row
                    label="Location"
                    value={
                      [open.proxyCity, open.proxyRegion, open.proxyZip]
                        .filter(Boolean)
                        .join(", ") || "—"
                    }
                  />
      ...
      <RankingScreenshotDialog
        recordId={shotId}
        endpoint="/api/audit-logs/{id}/screenshot-url"
        onClose={() => setShotId(null)}
        title="Audit screenshot"
        subtitle={
          open
            ? `${open.platform ?? ""} · ${open.keywordText ?? ""}`
            : undefined
        }
      />
    </div>
