 ...
 */
router.get("/session-breakdown", requireViewer, async (req, res) => {
  try {
    ...
    const breakdown = {
      ...
      type1: {
        ...
        description:
          "Primary geo-targeted AEO prompt searches. 100% search rate across all plans.",
        percentage: 60,
      ...
      type2: {
        ...
        description:
          "Backlink prompt searches. Backlinks are only made off of 1st/primary keywords.",
        percentage: 10,
      ...
      discrepancyReports: [
        {
          id: 1,
          label: "Business name verification",
          description: "Verify business name matches GBP listing exactly",
        },
        {
          id: 2,
          label: "First choice word verification",
          description:
            "Confirm primary keyword (1st word) is being used correctly",
        },
        {
          id: 3,
          label: "Total # of AEO searches / day / per word",
          description:
            "Including data re: randomization and alteration across AI platforms",
        },
        {
          id: 4,
          label: "1 search per device",
          description:
            "Maximum 1 AEO prompt search per device per day (daily rotation)",
        },
        {
          id: 5,
          label: "Popular point data",
          description:
            "Track popularity signals across Gemini, ChatGPT, and Perplexity",
        },
        {
          id: 6,
          label: "Direct popup data",
          description: "Monitor direct AI result popup appearances per keyword",
        },
        {
          id: 7,
          label: "Cross client data",
          description: "Cross-reference AEO performance data across clients",
        },
        {
          id: 8,
          label: "Google map rank location",
          description: "Via Local Falcon API — track GBP map ranking position",
        },
      ],
 ...
 */
router.get("/business", requireViewer, async (req, res) => {
  try {
    ...
    const devRows = await db
      ...
      .from(sessionsTable)
      .leftJoin(
        sql`devices ON devices.id = ${sessionsTable.deviceId}`,
        sql`true`,
      )
    ...
    const cacheRow = await db
      .select()
      .from(farmMetrics)
      .where(eq(farmMetrics.key, "cache_clearing"))
      .limit(1);
    const cacheValue = cacheRow[0]?.value
      ? parseFloat(cacheRow[0].value)
      : null;
    const cacheTarget = cacheRow[0]?.targetValue
      ? parseFloat(cacheRow[0].targetValue)
      : 100;
    ...
    const fmRows = await db
      .select()
      .from(farmMetrics)
      .where(inArray(farmMetrics.key, KEYS));
    ...
    const targets: Record<string, number> = {
      ...
      ip_rotation: 90,
      cache_clearing: 100,
      prompt_exec_accuracy: 95,
      volume_search_accuracy: 98,
    };
    for (const row of fmRows) {
      targets[row.key] = row.targetValue
    ...
    const kwByClient = Object.fromEntries(
      kwRows.map((r) => [r.clientId, Number(r.cnt)]),
    );

    // Group device detail rows by clientId for the device list per client
    const devicesByClient: Record<
      number,
      { deviceId: number; identifier: string; model: string }[]
    > = {};
    ...
    const result = clients.map((client) => {
      ...
      const deviceRotation =
        withDevice > 0
          ? Math.round((Math.min(uniqueDevs, withDevice) / withDevice) * 100)
          : null;
      const ipRotation =
        withProxy > 0
      ...
      const promptAccuracy =
        total > 0 ? Math.round((withPrompt / total) * 100) : null;
      ...
      return {
        ...
        deviceRotation: {
          ...
          uniqueDevices: uniqueDevs,
          withDevice,
          target: targets.device_rotation,
        },
        ipRotation: {
          value: ipRotation,
          uniqueProxies,
          withProxy,
          target: targets.ip_rotation,
        },
        cacheClearing: {
          value: cacheValue,
          isManual: true,
        ...
        promptAccuracy: {
          ...
          withPrompt,
          total,
          target: targets.prompt_exec_accuracy,
        },
        volumeAccuracy: {
          value: volumeAccuracy,
          actual: total,
          monthlyTarget,
          target: targets.volume_search_accuracy,
        },
      };
    });
 ...
 */
router.get("/performance", requireViewer, async (req, res) => {
  try {
    // ── Total session count ────────────────────────────────────────────────
    ...
    const [devRow] = await db
      ...
      .from(sessionsTable)
      .where(isNotNull(sessionsTable.deviceId));
    const [uniqDevRow] = await db
      .select({ c: sql<number>`COUNT(DISTINCT ${sessionsTable.deviceId})` })
      .from(sessionsTable)
      .where(isNotNull(sessionsTable.deviceId));
    ...
    const [proxyRow] = await db
      .select({ count: count() })
      .from(sessionsTable)
      .where(isNotNull(sessionsTable.proxyId));
    const [uniqProxyRow] = await db
      .select({ c: sql<number>`COUNT(DISTINCT ${sessionsTable.proxyId})` })
      .from(sessionsTable)
      .where(isNotNull(sessionsTable.proxyId));
    ...
    const [promptRow] = await db
      .select({ count: count() })
      .from(sessionsTable)
      .where(isNotNull(sessionsTable.promptText));
    const withPrompt = Number(promptRow.count);
    const promptAccuracy =
      total > 0 ? Math.round((withPrompt / total) * 100) : 0;
    ...
    res.json({
      ...
      deviceRotation: {
        ...
        uniqueDevices,
        withDevice,
        target: targets["device_rotation"] ?? "80",
        updatedAt: updatedAts["device_rotation"],
      },
      ipRotation: {
        value: ipRotation,
        uniqueProxies,
        withProxy,
        target: targets["ip_rotation"] ?? "90",
        updatedAt: updatedAts["ip_rotation"],
      },
      cacheClearing: {
        value: cacheValue ? parseFloat(cacheValue) : null,
        target: targets["cache_clearing"] ?? "100",
        updatedAt: updatedAts["cache_clearing"],
        isManual: true,
      },
      promptAccuracy: {
        value: promptAccuracy,
        withPrompt,
        total,
        target: targets["prompt_exec_accuracy"] ?? "95",
        updatedAt: updatedAts["prompt_exec_accuracy"],
      },
      volumeAccuracy: {
        value: volumeAccuracy,
        actual: total,
        targetCount: monthlyTarget,
        target: targets["volume_search_accuracy"] ?? "98",
        updatedAt: updatedAts["volume_search_accuracy"],
      },
    });
  } catch (err) {
 ...
 */
router.patch("/performance/:key", requireAdmin, async (req, res) => {
  try {
