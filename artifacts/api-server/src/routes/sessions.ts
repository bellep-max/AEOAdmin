...
router.get("/", requireSalesAllowed, async (req, res) => {
  try {
    ...
    if (businessId)
      conditions.push(eq(sessionsTable.businessId, parseInt(businessId)));
    if (campaignId)
      conditions.push(eq(sessionsTable.campaignId, parseInt(campaignId)));
    if (keywordId)
      conditions.push(eq(sessionsTable.keywordId, parseInt(keywordId)));
    if (deviceId)
      conditions.push(eq(sessionsTable.deviceId, parseInt(deviceId)));
    if (platform) conditions.push(eq(sessionsTable.aiPlatform, platform));
    if (status) conditions.push(eq(sessionsTable.status, status));
    if (from)
      conditions.push(
        gte(sessionsTable.timestamp, parseFilterDate(from, "start")),
      );
    if (to)
      conditions.push(lte(sessionsTable.timestamp, parseFilterDate(to, "end")));
    ...
    const sessions = await db
      ...
      .leftJoin(
        businessesTable,
        eq(sessionsTable.businessId, businessesTable.id),
      )
      .leftJoin(
        clientAeoPlansTable,
        eq(sessionsTable.campaignId, clientAeoPlansTable.id),
      )
  ...
  try {
    ...
    const [session] = await db
      ...
      .values({
        ...
        followupText:
          ((body.followupText ?? body.followUp) as string | null) ?? null,
        ...
        aiPlatform: String(
          body.aiPlatform ?? body.platform ?? "gemini",
        ).toLowerCase(),
        screenshotUrl:
          (body.screenshotUrl as string | null | undefined) ?? null,
        deviceIdentifier:
          (body.deviceIdentifier as string | null | undefined) ?? null,
        proxyStatus: (body.proxyStatus as string | null | undefined) ?? null,
        proxySessionId:
          (body.proxySessionId as string | null | undefined) ?? null,
        proxyUsername:
        ...
        baseLatitude:
          body.baseLatitude != null ? Number(body.baseLatitude) : null,
        baseLongitude:
          body.baseLongitude != null ? Number(body.baseLongitude) : null,
        mockedLatitude:
          body.mockedLatitude != null ? Number(body.mockedLatitude) : null,
        mockedLongitude:
          body.mockedLongitude != null ? Number(body.mockedLongitude) : null,
        mockedTimezone:
          (body.mockedTimezone as string | null | undefined) ?? null,
        backlinksExpected:
          body.backlinksExpected != null
            ? Number(body.backlinksExpected)
            : null,
    ...
    for (const key of PATCHABLE_SESSION_FIELDS) {
      ...
      if (NUMERIC_FIELDS.has(key)) {
        ...
        if (Number.isNaN(d.getTime()))
          return res
            .status(400)
            .json({ error: `${key} must be a valid ISO 8601 string` });
        patch[key] = d;
...
router.patch("/:id/timestamp", requireExecutorToken, async (req, res) => {
  try {
    ...
    if (Number.isNaN(parsed.getTime())) {
      return res
        .status(400)
        .json({ error: "timestamp must be a valid ISO 8601 string" });
    }
    const [row] = await db
      .update(sessionsTable)
...
/* GET /api/sessions/:id/screenshot-url
   Same contract as /api/ranking-reports/:id/screenshot-url:
     - "s3://..." → pre-signed GET URL
     - "https://..." → returned as-is
     - local path → falls back to looking up any ranking_reports row whose
...
router.get("/:id/screenshot-url", requireViewer, async (req, res) => {
  ...
  try {
    const rows = await db
      .select({ url: sessionsTable.screenshotUrl })
      .from(sessionsTable)
      .where(eq(sessionsTable.id, id))
      .limit(1);
    if (rows.length === 0) {
    ...
    /* For local paths, try the basename-fallback into ranking_reports' s3
       uploads. Done before the s3:// branch so a sessions row that has an
       s3:// already (unlikely) still works through the direct branch below. */
    ...
    /* Still a local path with no s3 backfill found. */
    return res.json({ url: null, kind: "local", originalPath: raw });
  } catch (err) {
    req.log.error({ err, id }, "Error generating session screenshot URL");
  ...
  try {
    ...
    const [updated] = await db
      ...
      .set({
        ...
        hasFollowUp: followupText.trim().length > 0,
      })
      .where(eq(sessionsTable.id, id))
      .returning();
    if (!updated) return res.status(404).json({ error: "Session not found" });
...
router.get("/stress-test", requireViewer, async (req, res) => {
  try {
    ...
    const [geminiCount] = await db
      .select({ count: count() })
      .from(sessionsTable)
      .where(eq(sessionsTable.aiPlatform, "gemini"));
    const [chatgptCount] = await db
      .select({ count: count() })
      .from(sessionsTable)
      .where(eq(sessionsTable.aiPlatform, "chatgpt"));
    const [perplexityCount] = await db
      .select({ count: count() })
      .from(sessionsTable)
      .where(eq(sessionsTable.aiPlatform, "perplexity"));
    ...
    const followupRate =
      totalSessions > 0 ? Number(withFollowup.count) / totalSessions : 0.5;

    const platformDistribution = [
      {
        platform: "gemini",
        count: Number(geminiCount.count),
        percentage:
          totalSessions > 0
            ? (Number(geminiCount.count) / totalSessions) * 100
            : 33.3,
      },
      {
        platform: "chatgpt",
        count: Number(chatgptCount.count),
        percentage:
          totalSessions > 0
            ? (Number(chatgptCount.count) / totalSessions) * 100
            : 33.3,
      },
      {
        platform: "perplexity",
        count: Number(perplexityCount.count),
        percentage:
          totalSessions > 0
            ? (Number(perplexityCount.count) / totalSessions) * 100
            : 33.4,
      },
    ];
  ...
  async (req, res) => {
    try {
      ...
      if (!file) {
        return res
          .status(400)
          .json({ error: "No CSV file uploaded. Use form field 'file'." });
      }
      ...
      try {
        ...
        return res.status(400).json({
          error:
            "Failed to parse CSV. Ensure it's valid CSV with a header row.",
        });
      }
      ...
      if (rows.length === 0) {
        return res
          .status(400)
          .json({ error: "CSV file is empty (no data rows)." });
      }
      ...
      const kwMap = new Map<
        string,
        { keywordId: number; businessId: number | null }
      >();
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if (!lookup) {
          ...
          if (errors.length < 100) {
            errors.push({
              row: rowNum,
              reason: `Keyword not found: "${keywordText}" for campaign_id=${campaignId}`,
            });
          }
          continue;
        }
        ...
        const duration = csvRow.duration_s
          ? parseFloat(csvRow.duration_s)
          : null;
        ...
        const backlinksExpected = csvRow.backlinks_expected
          ? parseInt(csvRow.backlinks_expected, 10)
          : 0;
        ...
        if (timestampRaw && isNaN(timestamp.getTime())) {
          errors.push({
            row: rowNum,
            reason: `Invalid timestamp: "${timestampRaw}"`,
          });
          skipped++;
          continue;
        }
        ...
        toInsert.push({
          ...
          baseLatitude: csvRow.base_latitude
            ? parseFloat(csvRow.base_latitude)
            : null,
          baseLongitude: csvRow.base_longitude
            ? parseFloat(csvRow.base_longitude)
            : null,
          mockedLatitude: csvRow.mocked_latitude
            ? parseFloat(csvRow.mocked_latitude)
            : null,
          mockedLongitude: csvRow.mocked_longitude
            ? parseFloat(csvRow.mocked_longitude)
            : null,
      ...
      if (toInsert.length > 0) {
        for (let offset = 0; offset < toInsert.length; offset += BATCH_SIZE) {
          ...
          try {
            ...
            req.log.error(
              { err: batchErr },
              `Batch insert failed at offset ${offset}`,
            );
            for (let j = 0; j < batch.length; j++) {
              try {
                ...
                errors.push({
                  row: offset + j + 2,
                  reason: `DB insert error: ${singleMsg}`,
                });
                skipped++;
              }
            }
